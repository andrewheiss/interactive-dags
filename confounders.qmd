...
Our estimand now includes $Z$, but this joint distribution equation is complex. We need to break it smaller, more manageable pieces. Using the chain rule of probability (which applies to interventional distributions just as it does to observational ones), we can expand out @eq-marginalized:
...
- using an empty conditioning set, so there's no $z$ term
