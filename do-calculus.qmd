...
$P(y \mid \operatorname{do}(x)) \;\leadsto\; P(y \mid x)$
...
{\large
  {\color{gray}
    \overbrace{
      \underset{
        \substack{\textbf{Interventional} \\[3pt] \text{``Causation"}}
      }{{\color{black}\mathbf{E}(y \mid \operatorname{do}(x))\vphantom{\frac{1}{1}}}}
    }^{
      \mathclap{\small\substack{
        \text{The average} \\
        \text{population-level} \\
        \text{change in } y \text{ when} \\
        \textit{directly setting } x
      }}
    }
  }
  \quad \underset{\color{gray}\textit{\footnotesize(usually)}}{\neq} \quad
  {\color{gray}
    \overbrace{
      \underset{
        \substack{\textbf{Observational} \\[3pt] \text{``Correlation"}}
      }{{\color{black}\mathbf{E}(y \mid x)\vphantom{\frac{1}{1}}}}
    }^{
      \mathclap{\small\substack{
        \text{The average} \\
        \text{population-level} \\
        \text{change in } y \text{ when} \\
        \textit{passively observing } x
      }}
    }
  }
}
